...
const {
	default: ElmErrorJson,
} = require("./vite-plugins/elm/elm-error-json.js");
	...
	if (err.code === "EPERM") {
		console.error(
			[
				...
				`    This problem can be fixed by running the ${Terminal.pink(
					"npm init -y",
				)} command`,
				`    from your terminal.`,
				``,
				`    If the issue persists after running that command, please let us`,
				`    know in the Elm Land Discord (${Terminal.dim(
					"https://join.elm.land",
				)})`,
				"",
			].join("\n"),
	...
	try {
		...
		configFileWatcher.on("change", async () => {
			try {
				...
				if (
					config.app.router.useHashRouting !=
					oldConfig.app.router.useHashRouting
				) {
		...
		srcPagesAndLayoutsAndCustomizedFileWatcher.on("all", () => {
			generateElmFiles(config, server);
		});

		// Listen for any changes to customizable files, so defaults are recreated
		// if the customized versions are deleted
		let customizableFileFilepaths = Object.values(
			Utils.customizableFiles,
		).flatMap(({ filepaths }) =>
			filepaths.map((filepath) =>
				path.join(process.cwd(), "src", ...filepath.split("/")),
			),
		);
		...
		// let debug = false;
		try {
			debug = config.app.elm[mode()].debugger;
		} catch (_) {}
...
let generateElmFiles = async (config, server = undefined) => {
	try {
		...
		let layoutFilepaths = Files.listElmFilepathsInFolder(
			srcLayoutsFolderFilepath,
		);

		let pages = await Promise.all(
			pageFilepaths.map(async (filepath) => {
				let contents = await Files.readFromUserFolder(
					`src/Pages/${filepath}.elm`,
				);
		...
		let layouts = await Promise.all(
			layoutFilepaths.map(async (filepath) => {
				let contents = await Files.readFromUserFolder(
					`src/Layouts/${filepath}.elm`,
				);
		...
		let view = await Files.readFromUserFolder("src/View.elm").catch(
			(_) => null,
		);
		...
		let errors = await validate({
			...
			shared: await Files.readFromUserFolder("src/Shared.elm").catch(
				(_) => null,
			),
			sharedModel: await Files.readFromUserFolder("src/Shared/Model.elm").catch(
				(_) => null,
			),
			sharedMsg: await Files.readFromUserFolder("src/Shared/Msg.elm").catch(
				(_) => null,
			),
			effect: await Files.readFromUserFolder("src/Effect.elm").catch(
				(_) => null,
			),
			view,
		});
		...
		if (errors.length === 0) {
			...
			let layoutFilepathSegments = layoutFilepaths.map((filepath) =>
				filepath.split("/"),
			);
...
const customize = async (filepaths) => {
	await Promise.all(
		filepaths.map(async (filepath) => {
			...
			try {
				await Files.remove(
					path.join(process.cwd(), ".elm-land", "src", ...filepath.split("/")),
				);
			} catch (_) {
...
const syncCustomizableFiles = async () => {
	let defaultFilepaths = Object.values(Utils.customizableFiles).flatMap(
		(obj) => obj.filepaths,
	);

	await Promise.all(
		defaultFilepaths.map(async (filepath) => {
			let fileInUsersSrcFolder = path.join(
				process.cwd(),
				"src",
				...filepath.split("/"),
			);
			let fileInTemplatesFolder = path.join(
				__dirname,
				"templates",
				"_elm-land",
				"customizable",
			...
			let fileInElmLandSrcFolder = path.join(
				process.cwd(),
				".elm-land",
				"src",
				...filepath.split("/"),
...
const handleViteBuildErrors = (err) => {
	...
	try {
		...
		// Provide helpful error for missing NPM dependencies
			...
			let [dependencyName, fileImportingPackage] = message
				.split("failed to resolve import ")[1]
				.split(' from "');
			fileImportingPackage = fileImportingPackage.split('".')[0];

			message = [
				`    The file ${Terminal.cyan(fileImportingPackage)} tried to import`,
				`    an NPM package named ${Terminal.pink(
					dependencyName,
				)}, but it wasn't found.`,
				"",
				`    Make sure to run ${Terminal.cyan(
					"npm install",
...
const generateHtml = async (config) => {
	...
	let htmlAttributes = toAttributeString(
		attempt(() => config.app.html.attributes.html),
	);
	let headAttributes = toAttributeString(
		attempt(() => config.app.html.attributes.head),
	);
	...
	let titleTags = attempt((_) => config.app.html.title)
		...
		: [];
	let metaTags = toSelfClosingHtmlTags(
		"meta",
		[{ name: "elm-land", content: "0.19.2" }].concat(
			attempt((_) => config.app.html.meta),
		),
	);
	let linkTags = toSelfClosingHtmlTags(
		"link",
		attempt((_) => config.app.html.link),
	);
	let scriptTags = toHtmlTags(
		"script",
		attempt((_) => config.app.html.script),
	);
